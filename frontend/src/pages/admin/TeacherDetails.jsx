...
const TeacherDetails = () => {
  ...
  const handlePhotoChange = async (e) => {
    ...
    try {
      ...
      formData.append('folder', 'teacher_avatars');
      ...
      // Update the teacher's photo_url in the database
      const response = await axios.put(
        `${BASE_URL}/api/v1/admin/teacher/${teacherId}`,
      ...
      if (response.data.success) {
        // Update both formData and teacher state with the new photo URL
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const endpoint = `${BASE_URL}/api/v1/admin/teacher/${teacherId}`;
      ...
      if (response.data.success) {
        // Update teacher state with the response data
        setTeacher(response.data.teacher);
        // Update formData to match the updated teacher data
        setFormData({
          full_name: response.data.teacher.full_name,
          email: response.data.teacher.email,
          department: response.data.teacher.department,
          qualification: response.data.teacher.qualification,
          yearOfExperience: Number(response.data.teacher.yearOfExperience),
          photo_url: response.data.teacher.photo_url
        });
        setIsEditing(false);
